...
if (grepl(config$options$analysis_type, "wald", ignore.case = TRUE)) {
    ...
    grp1s <- lapply(1:nreps, function(i) {sample(samples[, "sample"], length(grp1), replace = TRUE)})
    grp2s <- lapply(1:nreps, function(i) {sample(samples[, "sample"], length(grp2), replace = TRUE)})
}
...
if (grepl(config$options$analysis_type, "glm", ignore.case = TRUE)) {
    ...
    resamples <- lapply(1:nreps, function(i) {sample(samples[, "sample"], nrow(design), replace = TRUE)})
}
...
# Fit models
dml_list <- lapply(chrs, function(chr) {
