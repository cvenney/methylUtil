...
args <- commandArgs(T)
# setwd("/Volumes/MacHD/BernatchezProjects/sasa_epi/methylUtil") ; args <- c("10", "06_methylation_results/adults_6x6_min5_max20_groupWild_dmr_pval0.001.txt.gz", "05_bed_files/gene_w_promotors.bed")
...
sim <- mclapply(1:as.numeric(args[1]), mc.cores = detectCores(), function(s) {
    ...
    hits_dmr1 <- findOverlaps(sim_dmrs1, features)
