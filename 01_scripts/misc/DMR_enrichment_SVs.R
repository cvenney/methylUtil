...
setwd("~/Projects/sasa_epi/methylUtil") ; args <- c("1000", "06_methylation_results/adults_6x6_min5_max20_groupWild_dmr_pval0.001.txt.gz", "05_bed_files/ssa_SVs_high_conf_NCBIchrnames.bed")
...
del <- features[features$type == "<DEL>"]
...
sim <- mclapply(1:as.numeric(args[1]), mc.cores = detectCores(), function(s) {
    ...
    return(c(s, hits_del, hits_dup, hits_inv))
})
...
colnames(df) <- c("Replicate", "deletion_overlaps", "duplication_overlaps", "inversion_overlaps")

fwrite(df, sub("dmr_", "dmr_SV_overlaps", args[2]), sep = "\t", quote = FALSE)

del_hist <- ggplot(df, aes(x = deletion_overlaps)) +
    geom_histogram(binwidth = 1) +
    theme_classic() +
    labs(x = "Number of Overlapped Deletions", y = "Frequency", title = paste0("True Overlaps: N = ", real_del, "; p = ", sum(df$deletion_overlaps > real_del) / nrow(df))) +
...
save_plot(sub(".txt.gz", "_resampled_SV_overlap.png", args[2]), plot_grid(del_hist, dup_hist, inv_hist, ncol = 3, nrow = 1), base_height = 3.5, base_width = 8.5)
